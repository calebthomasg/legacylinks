...
import MemorialDiscoveryDetail from "@/components/trailhead/MemorialDiscoveryDetail";
...
function DiscoveryDetail({ item, onBack }: { item: DiscoveryItem; onBack: () => void }) {
  if (item.experience_type === "memorial") return <MemorialDiscoveryDetail experienceId={item.experience_id} fallbackTitle={item.title} fallbackDescription={item.description} onBack={onBack} />;
...
export default function TrailheadMap({ center, caches, myBoxes: _myBoxes, discoveryItems = [], myItems = [] }: Props) {
  ...
  return <div className="relative flex h-full min-h-[520px] w-full flex-col bg-sand lg:flex-row"><link href={`https://api.mapbox.com/mapbox-gl-js/v${MAPBOX_GL_VERSION}/mapbox-gl.css`} rel="stylesheet" /><Script src={MAPBOX_CSP_SCRIPT} strategy="afterInteractive" onLoad={() => setScriptReady(true)} onReady={() => setScriptReady(true)} onError={() => setError("Trailhead could not load Mapbox right now.")} /><div className="hidden h-full w-[380px] shrink-0 border-r border-night-sky/10 lg:block">{panel}</div><div className="shrink-0 border-b border-night-sky/10 bg-white p-2 lg:hidden"><div className="grid grid-cols-2 rounded-xl bg-sand p-1"><button type="button" onClick={() => setMobileView("map")} className={`rounded-lg px-4 py-2.5 text-sm font-bold transition ${mobileView === "map" ? "bg-white text-night-sky shadow-sm" : "text-night-sky/50"}`}>Map</button><button type="button" onClick={() => setMobileView("list")} className={`rounded-lg px-4 py-2.5 text-sm font-bold transition ${mobileView === "list" ? "bg-white text-night-sky shadow-sm" : "text-night-sky/50"}`}>List{mode === "explore" ? ` (${filtered.length})` : ""}</button></div></div><div className={`relative min-h-0 min-w-0 flex-1 ${mobileView === "list" ? "hidden lg:block" : "block"}`}><div ref={containerRef} className="absolute inset-0 h-full w-full" />{(!scriptReady || !loaded) && !error && <div className="absolute inset-0 flex items-center justify-center bg-sand text-sm font-semibold text-night-sky/60">Opening Trailhead…</div>}{error && <div className="absolute inset-0 flex items-center justify-center bg-sand px-6 text-center"><p className="font-semibold text-night-sky">{error}</p></div>}{active && <div className="absolute bottom-4 left-4 z-30 rounded-3xl bg-white/95 p-4 shadow-2xl"><p className="text-xs font-bold uppercase tracking-[.18em] text-coral">Adventure in progress</p><h2 className="mt-1 text-xl font-bold text-night-sky">{active.title}</h2><button onClick={() => { clearArea(); activeRef.current = null; setActive(null); }} className="mt-3 text-xs font-bold text-night-sky/60">Exit adventure</button></div>}</div><div className={`min-h-0 flex-1 bg-white lg:hidden ${mobileView === "list" ? "block" : "hidden"}`}>{panel}</div>{addView && <div className="absolute inset-0 z-50 flex items-center justify-center bg-night-sky/45 p-5"><div className="h-[min(760px,92vh)] w-full max-w-lg overflow-hidden rounded-[2rem] bg-white shadow-2xl">{addView === "memorial" ? <MemorialCreateForm onCancel={() => setAddView("chooser")} onCreated={() => { setAddView(null); refreshMine(); }} /> : <div className="p-6"><p className="text-xs font-bold uppercase tracking-[.18em] text-teal">My Treasure</p><h2 className="mt-2 text-2xl font-bold text-night-sky">What are you adding?</h2><p className="mt-2 text-sm leading-6 text-night-sky/55">Choose the kind of treasure you’re placing into the LegacyLink world.</p><div className="mt-5 grid gap-2"><button className="rounded-2xl border border-night-sky/15 p-4 text-left font-bold text-night-sky">Treasure Box <span className="block text-xs font-normal text-night-sky/45">Creator flow coming next</span></button><button onClick={() => setAddView("memorial")} className="rounded-2xl border border-teal bg-teal/10 p-4 text-left font-bold text-night-sky">Memorial / Headstone <span className="block text-xs font-normal text-night-sky/45">Create and publish a memorial now</span></button><button className="rounded-2xl border border-night-sky/15 p-4 text-left font-bold text-night-sky">Specialty Token <span className="block text-xs font-normal text-night-sky/45">Token placement flow coming next</span></button></div><button onClick={() => setAddView(null)} className="mt-5 w-full rounded-2xl bg-night-sky px-5 py-4 text-sm font-bold text-white">Close</button></div>}</div></div>}{editMemorialId && <div className="absolute inset-0 z-50 flex items-center justify-center bg-night-sky/45 p-5"><div className="h-[min(820px,94vh)] w-full max-w-lg overflow-hidden rounded-[2rem] bg-white shadow-2xl"><MemorialEditForm experienceId={editMemorialId} onCancel={() => setEditMemorialId(null)} onSaved={() => { setEditMemorialId(null); refreshMine(); }} /></div></div>}</div>;
}
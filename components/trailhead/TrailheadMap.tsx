...
function MyTreasureDetail({ item, onBack }: { item: MyTreasureItem; onBack: () => void }) {
...
function Panel({ mode, setMode, items, selectedItem, selectedCache, selectedMine, onSelect, onSelectMine, onBack, onBegin, myItems, onAdd, filter, setFilter }: { mode: "explore" | "mine"; setMode: (mode: "explore" | "mine") => void; items: DiscoveryItem[]; selectedItem: DiscoveryItem | null; selectedCache: TrailheadCache | null; selectedMine: MyTreasureItem | null; onSelect: (item: DiscoveryItem) => void; onSelectMine: (item: MyTreasureItem) => void; onBack: () => void; onBegin: (cache: TrailheadCache) => void; myItems: MyTreasureItem[]; onAdd: () => void; filter: Filter; setFilter: (filter: Filter) => void }) {
  ...
  if (selectedMine) return <aside className="flex h-full min-h-0 flex-col bg-white"><MyTreasureDetail item={selectedMine} onBack={onBack} /></aside>;
...
export default function TrailheadMap({ center, caches, myBoxes, discoveryItems = [], myItems = [] }: Props) {
  ...
  const initialize = useCallback(() => {
    ...
    try {
      ...
      setError(caught instanceof Error ? `Mapbox error: ${caught.message}` : "Trailhead could not load the map right now.");
    }

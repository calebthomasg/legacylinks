alter table public.physical_treasure_boxes add column if not exists experience_id uuid references public.physical_experiences(id) on delete set null;
create unique index if not exists physical_treasure_boxes_experience_id_key on public.physical_treasure_boxes(experience_id) where experience_id is not null;
...
begin
 ...
 select * into v_box from public.physical_treasure_boxes where nfc_public_token=p_nfc_public_token and owner_id=auth.uid() for update;
 ...
 if v_cache_id is null then
  ...
  update public.physical_treasure_boxes set cache_id=v_cache_id,updated_at=now() where id=v_box.id;
 else select adventure_id into v_adventure_id from public.caches where id=v_cache_id; end if;
 if v_experience_id is null then
  select id into v_experience_id from public.physical_experiences where cache_id=v_cache_id limit 1;
  ...
  update public.physical_treasure_boxes set experience_id=v_experience_id,updated_at=now() where id=v_box.id;
 end if;
...
begin
 ...
 update public.caches set title=trim(p_title),description=nullif(trim(coalesce(p_description,'')),''),difficulty=p_difficulty::smallint,terrain=p_terrain::smallint,updated_at=now() where id=v_box.cache_id and coalesce(owner_id,creator_id)=auth.uid();
 ...
 update public.physical_experiences set title=trim(p_title),description=nullif(trim(coalesce(p_description,'')),''),updated_at=now() where id=v_experience_id and coalesce(owner_id,creator_id)=auth.uid();
 ...
 update public.physical_treasure_boxes set setup_status='in_progress',updated_at=now() where id=v_box.id;
 return query select v_box.cache_id,v_experience_id,'in_progress'::text;
end;$$;

revoke all on function public.start_claimed_treasure_box_setup(uuid) from public; grant execute on function public.start_claimed_treasure_box_setup(uuid) to authenticated;
revoke all on function public.save_claimed_treasure_box_setup(uuid,text,text,integer,integer,double precision,double precision,integer) from public; grant execute on function public.save_claimed_treasure_box_setup(uuid,text,text,integer,integer,double precision,double precision,integer) to authenticated;
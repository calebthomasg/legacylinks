...
function typeLabel(type:PhysicalExperience["experience_type"]){
  if(type==="memorial")return "Memorial discovered";
  if(type==="token_hunt")return "Specialty token discovered";
  return "Treasure discovered";
}
function heading(type:PhysicalExperience["experience_type"]){
  if(type==="memorial")return "You made it to this memorial.";
  if(type==="token_hunt")return "You found something special.";
  return "You found it!";
}
function Shell({children}:{children:React.ReactNode}){
  return <main className="relative min-h-screen bg-sand px-6 py-24 sm:py-28">
    <Link href="/" className="absolute left-6 top-6 sm:left-10 sm:top-8" aria-label="LegacyLink home">
      <Image src="/images/ll-logo.svg" alt="LegacyLink" width={192} height={44} className="h-auto w-40 sm:w-48" priority/>
    </Link>
    <section className="mx-auto w-full max-w-2xl rounded-3xl border border-night-sky/10 bg-white p-8 shadow-sm sm:p-10">{children}</section>
  </main>;
}
...
export default async function PhysicalNfcPage({params}:Props){
  ...
  if(boxState){
    if(boxState.claim_status==="unclaimed"){
      return <Shell>
        <p className="text-sm font-semibold uppercase tracking-wide text-teal">New Treasure Box</p>
        <h1 className="mt-3 text-4xl font-bold tracking-tight text-night-sky">Your Treasure Box is ready for the trail.</h1>
        <div className="mt-8 rounded-2xl bg-sand p-6">
          <p className="text-xs font-bold uppercase tracking-[.14em] text-night-sky/40">Treasure Box</p>
          <p className="mt-2 font-mono text-2xl font-bold text-night-sky">{boxState.box_id}</p>
          <p className="mt-3 leading-7 text-night-sky/65">This box has not been claimed yet. Use the activation PIN from the setup card inside the box to connect it to your LegacyLink account.</p>
        </div>
        {isSignedIn
          ?<TreasureBoxActivationForm token={token} boxId={boxState.box_id}/>
          :<div className="mt-8">
            <p className="leading-7 text-night-sky/70">Sign in or create a LegacyLink account first. We’ll bring you back to this exact Treasure Box so you can enter its activation PIN.</p>
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              <Link href={withReturnPath("/login",returnPath)} className="button-primary inline-flex justify-center">Log in</Link>
              <Link href={withReturnPath("/signup",returnPath)} className="inline-flex items-center justify-center rounded-xl border border-night-sky/20 px-5 py-3 font-semibold text-night-sky">Create account</Link>
            </div>
          </div>}
      </Shell>;
    }
    ...
    if(boxState.is_owner&&boxState.setup_status!=="published"){
      const{data:setup,error:setupError}=await supabase
        .rpc("start_claimed_treasure_box_setup",{p_nfc_public_token:token})
        .maybeSingle<SetupState>();
      ...
      return <Shell>
        <p className="text-sm font-semibold uppercase tracking-wide text-teal">Set up your Treasure Box</p>
        <h1 className="mt-3 text-4xl font-bold tracking-tight text-night-sky">Give your box a place on the trail.</h1>
        <p className="mt-4 leading-7 text-night-sky/65">Start with the basics. Nothing you save here is public until you confirm placement, review the box, and publish it.</p>
        <TreasureBoxSetupForm token={token} boxId={setup.box_id} initial={setup}/>
      </Shell>;
    }
  }

  const{data:experience,error}=await supabase
    .rpc("get_physical_nfc_experience",{p_public_token:token})
  ...
  const statusDetail=nfcVerified
    ...
    :experience.experience_type==="treasure_box"&&find
      ?(find.is_first_find
        ...
        :"You had already logged this Treasure Box, so LegacyLink kept your original find.")
      :"This scan is recorded separately from simply viewing this place on Trailhead.";
  ...
  return <Shell>
    ...
    {isSignedIn
      ...
      :<div className="mt-8">
        <p className="leading-7 text-night-sky/70">You’ve reached this LegacyLink through a physical NFC tag. Log in or create an account so this discovery can be connected to you.</p>
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <Link href={withReturnPath("/login",returnPath)} className="button-primary inline-flex justify-center">Log in</Link>
          <Link href={withReturnPath("/signup",returnPath)} className="inline-flex items-center justify-center rounded-xl border border-night-sky/20 px-5 py-3 font-semibold text-night-sky">Create account</Link>
        </div>

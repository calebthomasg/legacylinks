...
export default async function TrailheadPage(){
 ...
 for(const token of owned){tokenItems.set(token.token_id,{experience_id:token.token_id,experience_type:"token_hunt",cache_id:null,title:token.name||token.token_code,description:`Traveling Token${token.mission?` · ${token.mission}`:""}`,status:token.status,visibility:token.status==="in_circulation"?"public":"private",latitude:null,longitude:null,created_at:token.circulation_started_at||new Date().toISOString()});}
 for(const token of carried){tokenItems.set(token.token_id,{experience_id:token.token_id,experience_type:"token_hunt",cache_id:null,title:token.name||token.token_code,description:`Traveling Token · Carrying from ${token.from_box_code||"a Treasure Box"}${token.mission?` · ${token.mission}`:""}`,status:"in_transit",visibility:"private",latitude:null,longitude:null,created_at:token.picked_up_at||new Date().toISOString()});}

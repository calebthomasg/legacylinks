...
export default async function TrailheadPage(){
 ...
 const[{data:profile},{data:caches,error},{data:myBoxes},{data:discoveryItems},{data:myItems},{data:carriedTokens},{data:ownedTokens},{data:isAdmin}]=await Promise.all([
  supabase.from("profiles").select("first_name").eq("id",user.id).single(),supabase.rpc("get_trailhead_caches"),supabase.rpc("get_my_treasure_boxes"),supabase.rpc("get_trailhead_discovery_items"),supabase.rpc("get_my_trailhead_items"),supabase.rpc("get_carried_traveling_tokens"),supabase.rpc("get_owned_traveling_tokens"),supabase.rpc("is_legacy_link_admin")]);
 const trailheadCaches=(caches??[]) as TrailheadCache[],ownedBoxes=(myBoxes??[]) as MyTreasureBox[],discoveries=(discoveryItems??[]) as DiscoveryItem[],baseItems=(myItems??[]) as MyTreasureItem[],carried=(carriedTokens??[]) as CarriedToken[],owned=(ownedTokens??[]) as OwnedToken[];
 const tokenItems=new Map<string,MyTreasureItem>();
 for(const t of owned)tokenItems.set(t.token_id,{experience_id:t.token_id,experience_type:"traveling_token",cache_id:null,title:t.name||t.token_code,description:t.mission,status:t.status,visibility:t.status==="in_circulation"?"public":"private",latitude:null,longitude:null,created_at:t.circulation_started_at||new Date().toISOString(),nfc_public_token:t.nfc_public_token,token_code:t.token_code,mission:t.mission,current_box_code:t.current_box_code,total_miles:t.total_miles,stop_count:t.stop_count});
 for(const t of carried)tokenItems.set(t.token_id,{experience_id:t.token_id,experience_type:"traveling_token",cache_id:null,title:t.name||t.token_code,description:t.mission,status:"in_transit",visibility:"private",latitude:null,longitude:null,created_at:t.picked_up_at||new Date().toISOString(),nfc_public_token:t.nfc_public_token,token_code:t.token_code,mission:t.mission,from_box_code:t.from_box_code,total_miles:t.total_miles,stop_count:t.stop_count});
 const ownedItems=[...tokenItems.values(),...baseItems];
 return <AppShell active="trailhead" userName={profile?.first_name||"Trailblazer"} userEmail={user.email} profileHref={profileHref} isAdmin={Boolean(isAdmin)} contentClassName="relative overflow-hidden bg-sand"><div className="relative h-[calc(100dvh-77px)] min-h-[560px] lg:h-screen lg:min-h-[640px]">{error?<div className="flex h-full items-center justify-center bg-sand px-6 text-center"><div className="max-w-md"><p className="text-sm font-semibold uppercase tracking-[0.2em] text-coral">Trailhead unavailable</p><h1 className="mt-3 text-2xl font-bold text-night-sky">We couldn’t load nearby adventures.</h1><p className="mt-3 leading-7 text-night-sky/65">Try refreshing Trailhead in a moment.</p></div></div>:<TrailheadMap center={DEFAULT_TRAILHEAD_CENTER} caches={trailheadCaches} myBoxes={ownedBoxes} discoveryItems={discoveries} myItems={ownedItems}/>}</div><TrailheadWelcome/></AppShell>;
}
